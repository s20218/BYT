...
public class Account {
	...
	public void addTimedPayment(String id, Integer interval, Integer next, Money amount, Bank tobank, String toaccount) {
		//it's necessary to check if the timed payment doesn't exist yet
		if (timedPaymentExists(id)){
			System.err.println("Timed payment with the given id already exists.");
		}
		else {
	...
	public void removeTimedPayment(String id) {
		//it's necessary to check if the timed payment exists, before deleting it
		if (!timedPaymentExists(id)){
			System.err.println("Timed payment with the given id does not exist.");
		}
		else {
	 ...
	 */
	public void tick() {
		//tp.tick() method was being executed twice
	...
	public void withdraw(Money money) {
		...
		if(content.getAmount() < money.getAmount()){
